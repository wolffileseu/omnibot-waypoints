...
# Usage: python obtool.py in console
#
...
########################################################################
NoScriptPath = ('../../et/incomplete_navs/no_script/')
NoScriptList = os.listdir(os.path.normpath(NoScriptPath))
WithScriptPath = ('../../et/incomplete_navs/with_script/')
WithScriptList = os.listdir(os.path.normpath(WithScriptPath))
PriorityPath = ('../../et/incomplete_navs/priority_maps/')
		...
		if os.path.isfile(Path+fname):
			...
			if ext[1]==".gm":
				if ext[0].find('_goals') == -1:
					...
					if ( s.find('Debug = 1') > 0 ):
						val = '1'
					if ( s.find('Debug = true') > 0):
						val = 'true'
					if (val.__len__() > 0):
						tempFile = Path+fname+'~~~'
						outp = open(tempFile, 'w')
						outtext = re.sub('Debug = '+val, 'Debug = 0', s)
						outp.write(outtext)
						outp.close()
						os.remove(origFile)
